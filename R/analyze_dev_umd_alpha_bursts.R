...
sink("../output/dev_umd_alpha_burst_duration_cycles_results.txt")
...
for(age in ages) {
      ...
      print(emmeans(m, pairwise ~ epoch|condition, type = 'response'))
}
...
sink("../output/dev_umd_alpha_burst_duration_seconds_results.txt")
...
for(age in ages) {
      ...
      m <- lmer(burst_duration ~ condition * epoch + (1+condition+epoch | subject), data = df)
      print(Anova(m, type = 3))
      print(emmeans(m, pairwise ~ condition|epoch, type = 'response'))
      print(emmeans(m, pairwise ~ epoch|condition, type = 'response'))
}
