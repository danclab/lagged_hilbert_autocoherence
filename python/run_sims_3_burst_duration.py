...
def gen_signal_bursts(T, trials, srate, f, snr_db, num_bursts, burst_duration):
    ...
    spacing[num_bursts > 1] = (T - total_burst_duration[num_bursts > 1]) / (num_bursts[num_bursts > 1] - 1)

    for trial in range(trials):
        # Generate burst start times without overlap
        burst_starts = np.array([i * (burst_duration[trial] + spacing[trial]) for i in range(int(num_bursts[trial]))])
...
for snr_idx, snr in enumerate(snrs):
    ...
    for f_idx, f in enumerate(brst_f):
        n = np.ones(n_trials)
        d_c = np.random.uniform(2, 5, size=n_trials)
        d = d_c / f
...
np.savez('../output/sims/burst_dur/sim_results',
         snrs=snrs,
         lags=lags,
         freqs=freqs,
         lfc=lfc,
...
d_bursts = np.zeros(len(snrs)*len(brst_f)*n_trials)
d_c_bursts = np.zeros(len(snrs)*len(brst_f)*n_trials)
...
for snr_idx in range(len(snrs)):
    for f_idx in range(len(brst_f)):
        ...
        for i in range(n_trials):
            ...
            d_bursts[idx] = brst_d[snr_idx, f_idx, i]
            d_c_bursts[idx] = brst_d_c[snr_idx, f_idx, i]

            b_lc_hilbert = t_lhc[lc_f_idx, :]
            p0 = [np.median(lags), 3]
